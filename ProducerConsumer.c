...
int buff[5];   // 5 is the size of the buffer
sem_t s, e, f; // Data Type of Semaphore (s-BinarySemaphore, e-Empty, f-Filled)
int data = 99;  // Data to be entered in the buffer

// Producer Function
void *producer(void *arg)
{
    sem_wait(&e); // Allows the producer to enter the critical section
    sem_wait(&s); // Allows only one process to access the buffer

    int data = rand() % 5; // rand() generates a random number
    buff[in] = data;       // Random number has been entered in the buffer
